...
const waveVertex = `precision highp float;attribute vec2 position;attribute vec2 uv;attribute vec2 iOffset;attribute vec2 iScale;attribute float iOpacity;varying vec2 vUv;varying float vOpacity;void main(){vUv=uv;vOpacity=iOpacity;gl_Position=vec4(iOffset+position*iScale,0.0,1.0);}`;
const waveFragment = `precision highp float;varying vec2 vUv;varying float vOpacity;uniform float uRings;const float PI=3.141592653589793;const float EDGE=0.006737947;void main(){vec2 p=vUv*2.0-1.0;float r=dot(p,p);if(r>1.0)discard;float brush=(exp(-r*5.0)-EDGE)/(1.0-EDGE);brush*=0.55+0.45*cos(sqrt(r)*PI*2.0*uRings);gl_FragColor=vec4(vec3(brush*vOpacity*vOpacity),1.0);}`;
...
const RippleDistortion = ({ src = 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?q=80&w=3416&auto=format&fit=crop', brushSize = 150, strength = 0.2, swirl = 1, rings = 4, spread = 5, fade = 3, spacing = 15, dispersion = 0, glint = 0, tint = '#d4a574', tintAmount = 0.1, grayscale = true, highlightColor = '#ffffff', trigger = 'hover', clickStrength = 2, quality = 'low', enabled = true, className = '', style }) => {
  ...
  useEffect(() => {
    ...
    const canvas = gl.canvas; canvas.style.width = '100%'; canvas.style.height = '100%'; canvas.style.display = 'block'; mount.appendChild(canvas);
    ...
    let disposed = false;
    const image = new window.Image(); image.crossOrigin = 'anonymous'; image.decoding = 'async';
    image.onload = () => { if (disposed) return; imageTexture.image = image; compositeUniforms.uTextureSize.value = [image.naturalWidth || 1, image.naturalHeight || 1]; }; image.src = src;
    const offsets = new Float32Array(MAX_WAVES * 2); const scales = new Float32Array(MAX_WAVES * 2); const opacities = new Float32Array(MAX_WAVES);
    const waves = Array.from({ length: MAX_WAVES }, () => ({ x: 0, y: 0, scale: START_SCALE, target: START_SCALE, size: 1, opacity: 0 })); let current = 0;
    ...
    const localPoint = (cx, cy) => { const rect = mount.getBoundingClientRect(); if (rect.width === 0 || rect.height === 0) return null; if (cx < rect.left || cx > rect.right || cy < rect.top || cy > rect.bottom) return null; return [cx - rect.left, rect.height - (cy - rect.top)]; };
    let prevX = 0, prevY = 0;
    const onMove = e => { const cfg = configRef.current; if (!cfg.enabled || reduceMotion || cfg.trigger === 'click') return; const p = localPoint(e.clientX, e.clientY); if (!p) return; const step = Math.max(1, cfg.spacing); if (Math.abs(p[0] - prevX) > step || Math.abs(p[1] - prevY) > step) { setNewWave(p[0], p[1], 1); prevX = p[0]; prevY = p[1]; } };
    const onDown = e => { const cfg = configRef.current; if (!cfg.enabled || reduceMotion || cfg.trigger === 'hover') return; const p = localPoint(e.clientX, e.clientY); if (!p) return; setNewWave(p[0], p[1], Math.max(1, cfg.clickStrength)); };
    window.addEventListener('pointermove', onMove, { passive: true }); window.addEventListener('pointerdown', onDown, { passive: true });
    let raf = 0, prevTime = 0;
    const loop = now => { raf = requestAnimationFrame(loop); const delta = prevTime ? Math.min(0.05, (now - prevTime) / 1000) : 0; prevTime = now; const cfg = configRef.current; const growth = reduceMotion ? 0 : 1 - Math.exp(-delta * 1.09); const decay = reduceMotion ? 1 : Math.exp((-delta * LIFE_CONSTANT) / Math.max(0.15, cfg.fade));
      for (let i = 0; i < MAX_WAVES; i++) { const w = waves[i]; if (w.opacity <= 0) { opacities[i] = 0; continue; } w.opacity *= decay; w.scale += (w.target - w.scale) * growth; if (w.opacity < 0.002) { w.opacity = 0; opacities[i] = 0; continue; } const half = (w.scale * w.size) / 2; offsets[i * 2] = (w.x / width) * 2 - 1; offsets[i * 2 + 1] = (w.y / height) * 2 - 1; scales[i * 2] = (half / width) * 2; scales[i * 2 + 1] = (half / height) * 2; opacities[i] = w.opacity; }
